  <!-- Menu -->

  <nav class="navbar navbar-expand-sm border-bottom" style="background-color: white; width:100%; heigth:10%;" >
  <a  class="navbar-brand" href="#" style="color:indigo" onMouseover="this.style.color='black'" onMouseout="this.style.color='indigo'">
  <img width="75px" class="pl-2" src="./img/prueba.gif"/>Kiwi Dreamers </a>
  <button class="navbar-toggler bg-light" type="button" data-toggle="collapse" data-target="#navbarSupportedContent" aria-controls="navbarSupportedContent" aria-expanded="false" aria-label="Toggle navigation">
    <span class="navbar-toggler-icon"></span>
  </button>

  <div class="collapse navbar-collapse text-center" id="navbarSupportedContent" onMouseover="this.style.color='blue'" onMouseout="this.style.color='red'">
    <ul class="navbar-nav mr-auto">
      <li class="nav-item active">
        <a class="nav-link rounded-pill" style="color:indigo" href="#" onMouseover="this.style.backgroundColor='indigo'; this.style.color='white'" onMouseout="this.style.backgroundColor='white';this.style.color='indigo'"> Contacto <span class="sr-only">(current)</span></a>
      </li>
      <li class="nav-item">
        <a class="nav-link rounded-pill" style="color:indigo" href="#" onMouseover="this.style.backgroundColor='indigo'; this.style.color='white'" onMouseout="this.style.backgroundColor='white';this.style.color='indigo'">Acerca de</a>
      </li>
    </ul>
    <form class="form-inline my-2 my-lg-0 text-center">
      <button class="rounded-pill btn-outline-dark my-2 my-sm-0 ml-1 " type="submit" onMouseover="this.style.backgroundColor='indigo'; this.style.color='white'" onMouseout="this.style.backgroundColor='white';this.style.color='indigo'">Registrar</button>
      <button class="rounded-pill btn-outline-dark my-2 my-sm-0 ml-1" type="submit" onMouseover="this.style.backgroundColor='indigo'; this.style.color='white'" onMouseout="this.style.backgroundColor='white'; this.style.color='indigo'">Iniciar Sesion</button>
    </form>
  </div>
</nav>

